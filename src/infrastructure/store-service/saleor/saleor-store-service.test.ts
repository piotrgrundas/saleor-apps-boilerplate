...
import { createSaleorStoreService } from "./saleor-store-service";
...
describe("createSaleorStoreService.getAppId", () => {
  it("returns app id on success", async () => {
    // given
    mockFetch(jsonResponse({ data: { app: { id: "app-123" } } }));
    const service = createSaleorStoreService(createMockJwksService());

    // when
    const result = await service.getAppId({ apiUrl: API_URL, token: TOKEN });
  ...
  it("sends correct request", async () => {
    ...
    // when
    await service.getAppId({ apiUrl: API_URL, token: TOKEN });
  ...
  it("returns STORE_REQUEST_ERROR on network error", async () => {
    ...
    expect(result._unsafeUnwrapErr()[0].code).toBe("STORE_REQUEST_ERROR");
  ...
  it("returns STORE_GRAPHQL_ERROR on graphql errors", async () => {
    ...
    // then
    expect(result.isErr()).toBe(true);
    expect(result._unsafeUnwrapErr()[0].code).toBe("STORE_GRAPHQL_ERROR");
  ...
  it("returns STORE_APP_NOT_FOUND_ERROR when app is null", async () => {
    ...
    // then
    expect(result.isErr()).toBe(true);
    expect(result._unsafeUnwrapErr()[0].code).toBe("STORE_APP_NOT_FOUND_ERROR");
  });
...
describe("createSaleorStoreService.verifyWebhook", () => {
  it("returns webhook data on valid headers and signature", async () => {
    // given
    const service = createSaleorStoreService(createMockJwksService());

    // when
    const result = await service.verifyWebhook({ headers: validHeaders, body: "payload" });
  ...
  it("returns STORE_WEBHOOK_HEADERS_ERROR when headers are missing", async () => {
    ...
    // when
    const result = await service.verifyWebhook({ headers: {}, body: "payload" });

    // then
    expect(result.isErr()).toBe(true);
    expect(result._unsafeUnwrapErr()[0].code).toBe("STORE_WEBHOOK_HEADERS_ERROR");
  });
  ...
  it("returns STORE_WEBHOOK_SIGNATURE_ERROR when signature verification fails", async () => {
    ...
    const failingJwks: JWKSService = {
      verify: async () =>
        err([{ code: "JWKS_NO_MATCHING_KEY_ERROR", message: "No matching key" }]),
    };
    const service = createSaleorStoreService(failingJwks);
    ...
    expect(error.code).toBe("STORE_WEBHOOK_SIGNATURE_ERROR");
    const cause = (error.details as { cause: { code: string }[] }).cause;
    expect(cause[0].code).toBe("JWKS_NO_MATCHING_KEY_ERROR");
  });
});
